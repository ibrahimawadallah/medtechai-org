...
function calcFallback(string $calc, string $data): array {
    ...
    switch ($calc) {
        ...
        case 'timi':
            return ['result' => 'TIMI Risk Score', 'score' => 'Requires clinical data', 'interpretation' => 'Provide: age ≥65, ≥3 CAD risk factors, aspirin use, severe angina, STEMI, st depression, elevated enzymes, hypertension, known CAD/SVD.', 'riskCategory' => 'ACS Risk Assessment', 'calculationMethodology' => 'TIMI: Age≥65(3), ≥3 risk factors(3), Aspirin(4), Recent STEMI(6), ST depression(2), Elevated enzymes(4), Severe angina(2), HTN(2), Known CAD/SVD(2). Range 0-14.', 'variablesUsed' => ['Age', 'Risk factors', 'Aspirin', 'STEMI', 'ST depression', 'Enzymes', 'Angina', 'HTN', 'CAD/SVD'], 'clinicalApplication' => 'Risk stratification for UA/NSTEMI, guides antiplatelet therapy', 'evidenceLevel' => 'Validated in TIMI trials', 'caveats' => ['Limited utility in STEMI patients', 'Risk factors must be documented']];
        case 'grace':
            return ['result' => 'GRACE Score', 'score' => 'Requires clinical data', 'interpretation' => 'Provide: age, heart rate, systolic BP, creatinine, Killip class, cardiac arrest, ST deviation, elevated enzymes.', 'riskCategory' => 'ACS Mortality Prediction', 'calculationMethodology' => 'GRACE uses 8 variables with continuous and categorical weighting. Predicted 6-month mortality.', 'variablesUsed' => ['Age', 'Heart rate', 'SBP', 'Creatinine', 'Killip', 'Arrest', 'ST deviation', 'Enzymes'], 'clinicalApplication' => 'Guides in-hospital management and discharge planning', 'evidenceLevel' => 'Extensively validated', 'caveats' => ['Requires accurate hemodynamic data', 'Mortality prediction more reliable than event prediction']];
        case 'sofa':
            return ['result' => 'SOFA Score', 'score' => 'Requires clinical data', 'interpretation' => 'Provide: PaO2/FiO2, platelets, bilirubin, hypotension/vasopressors, Glasgow Coma Scale, urine output.', 'riskCategory' => 'Organ Failure Assessment', 'calculationMethodology' => 'SOFA assesses 6 organ systems (respiratory, coagulation, liver, cardiovascular, CNS, renal). Each 0-4 points.', 'variablesUsed' => ['PaO2/FiO2', 'Platelets', 'Bilirubin', 'BP/Vasopressors', 'GCS', 'Urine output'], 'clinicalApplication' => 'Sepsis severity, ICU monitoring', 'evidenceLevel' => 'Sepsis-3 definition standard', 'caveats' => ['Requires worst values in first 24h', 'Baseline organ dysfunction affects interpretation']];
        default:
