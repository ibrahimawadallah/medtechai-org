# Outreach Script Template (Days 11-12)

## Primary Outreach Message

**Subject**: Quick AI tool that saves Abu Dhabi pharmacies 10+ hours weekly on drug research

**Message (WhatsApp/LinkedIn)**:

> Hi [Name],
> 
> I'm with MedTechAI, based in Abu Dhabi. We've built an AI-powered drug reference tool that helps pharmacy teams save 10+ hours per week on medication research while reducing prescription errors and staying fully compliant with UAE regulations.
> 
> Our Professional plan is AED 109/month (cancel anytime, no credit card required to start). It includes local drug pricing in AED, Arabic language support, and UAE-specific interaction databases.
> 
> It takes just 60 seconds to show you how it works. Would you be open to a 5-minute demo this week?
> 
> - 50+ AI clinical tools in one platform
> - Drug interactions, dosing, warnings - instant access
> - Arabic language support
> - Free plan available for testing
> 
> Best regards,
> [Your name]
> MedTechAI Organization
> Abu Dhabi, UAE

## Email Template

**Subject**: AI-Powered Drug Reference Tool - Saving Abu Dhabi Clinics 10+ Hours Weekly

**Body**:

> Hello [Name],
> 
> I hope this message finds you well. I'm reaching out from MedTechAI, an Abu Dhabi-based organization building AI-powered clinical tools for healthcare professionals across the Arab world.
> 
> We've developed a Drug Search & Insight tool that:
> • Saves 10+ hours weekly on medication research with UAE-specific pricing (AED)
> • Reduces prescription errors with real-time drug interaction checking (UAE-focused database)
> • Ensures UAE Ministry of Health compliance is automatically maintained
> • Provides Arabic language support for all drug information
> 
> The tool is already being used by 2,500+ pharmacy professionals in the UAE, and we're looking to expand partnerships with leading healthcare institutions like yours.
> 
> Our Professional plan is AED 109/month (cancel anytime, no credit card required to start with our Free tier).
> 
> Would you be open to a 5-10 minute demonstration? I can show you exactly how it works and how it could benefit your team.
> 
> We offer a free plan for initial testing, and our Professional tier is $29/month (cancel anytime, no credit card required to start).
> 
> Looking forward to your response.
> 
> Best regards,
> [Your name]
> MedTechAI Organization
> contact@medtechai.net
> Abu Dhabi, UAE

## Follow-up Message (if no response after 3 days)

> Hi [Name],
> 
> Just checking in - I sent a message about our AI drug research tool that could save your team significant time each week.
> 
> Would you have 5 minutes for a quick demo? No commitment required.
> 
> - [Your name]

## Demo Call Script (When they respond)

1. **Opening (30 seconds)**
   > "Thanks for your time. I'll keep this brief - just 5 minutes to show you how our tool works and the value it provides."

2. **Problem Identification (1 minute)**
   > "What's your biggest challenge with drug information lookup today?"

3. **Demo (3 minutes)**
   - Show the Drug Search tool
   - Show how it returns mechanism, interactions, dosing, warnings
   - Show the time comparison (manual vs AI-powered)

4. **Value Proposition (1 minute)**
   > "Our users report saving 10+ hours weekly and catching 3-4 interaction alerts they might have missed."

5. **Close (30 seconds)**
   > "Would you like to try the free plan with your team? I can set that up now."

## Key Questions to Ask During Demo Calls
- "What's the biggest pain point in your current drug lookup workflow?"
- "How do you currently check for drug interactions?"
- "What's your process for staying updated on UAE medication regulations?"
- "How much time does your team spend weekly on medication research?"
- "Would you be interested in a team trial with our Professional plan?"